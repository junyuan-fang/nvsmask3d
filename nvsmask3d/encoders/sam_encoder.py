...
class SamNetWork(BaseImageEncoder):
    def __init__(
        ...
        self.num_random_rounds = (
            config.num_random_rounds
        )  # Access num_random_rounds from config
        self.num_selected_points = (
            config.num_selected_points
        )  # Access num_selected_points from config
    ...
    def set_image(self, image: torch.Tensor):
        ...
        self.predictor.set_torch_image(
            transformed_image, original_image_size=self.original_size
        )
    ...
    def get_best_mask(self, point_coords: torch.Tensor) -> Tuple[torch.Tensor, list]:
        ...
        for i in range(self.num_random_rounds):
            ...
            selected_coords = selected_coords[: self.num_selected_points].unsqueeze(
                0
            )  # [shape: (1, num_selected_points, 2)]
            selected_coords = self.predictor.transform.apply_coords_torch(
                selected_coords, original_size=self.original_size
            )
            ...
            point_labels = torch.ones(
                (
                    1,
                    self.num_selected_points
                    if selected_coords.shape[1] > self.num_selected_points
                    else selected_coords.shape[1],
                ),
            ...
            try:
                (
                    ...
                    scores,
                    _,
                ) = self.predictor.predict_torch(  # BxCxHxW format     scores torch.Size([1, 3])
        ...
        if best_mask is None:
            best_mask = torch.zeros(
                self.original_size, dtype=torch.bool, device=self.model.device
            )
    ...
    @staticmethod
    def mask2box_multi_level(
        mask_i: torch.Tensor, level: int, expansion_ratio: float = 1
    ):
        """
...
if __name__ == "__main__":
    ...
    # Step 1: 加载图像
    image = Image.open(
        "/home/fangj1/Code/nerfstudio-nvsmask3d/nvsmask3d/data/replica/office0/color/7.jpg"
    ).convert("RGB")  # 确保是RGB三通道
    ...
    point_coords = torch.tensor(
        [[210, 238], [203, 252], [214, 257], [189, 245], [228, 250]], device="cuda:0"
    )
    ...
    print("Marked image saved as tests/1.png")
    # Step 5: 进行遮罩预测
    ...
    binary_mask_np = (binary_mask > 0).astype(
        "uint8"
    ) * 255  # 将 True 转换为 255，False 转换为 0
    ...
    print("Image with mask saved as tests/2.png")
