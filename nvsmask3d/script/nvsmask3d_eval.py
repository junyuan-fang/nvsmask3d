...
import torch
from nvsmask3d.utils.utils import (
    make_square_image,
    save_predictions,
...
class ComputeForAP:  # pred_masks.shape, pred_scores.shape, pred_classes.shape #((237360, 177), (177,), (177,))
    ...
    top_k: int = 15
    visibility_score: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] = (
        lambda num_visible_points, bounding_box_area: num_visible_points
        * bounding_box_area
    )
        ...
        with torch.no_grad():
            ...
            for i, scene_name in tqdm(
                ...
                if (
                    ...
                    output_dir = "/results/" + self.run_name_for_wandb  ### written?
            ...
            if self.inference_dataset == "replica":
                ...
                file_name = os.path.join(
                    folder_name, f"{timestamp}_{self.run_name_for_wandb}.json"
                )
    ...
    def pred_classes_with_sam(self, scene_name=""):
        ...
        """
        print(f"pred_classes_with_sam {self.top_k} views")
        class_agnostic_3d_mask = self.model.points3D_mask
        seed_points_0 = self.model.seed_points[0].cuda()
        ...
        for i in tqdm(range(cls_num), desc="Inferenceing objects", total=cls_num):
            ...
            # kind = "crop"
            kind = self.kind
            interp_kind = kind if self.interp_kind == "same" else self.interp_kind
            rgb_outputs = []
                        ...
                        if min_u < max_u and min_v < max_v:
                            ...
                            if self.interpolate_n_rgb_camera > 0:
                                if interp_kind == "crop":
                                    ...
                                    # no logic for now
                                    pass
                                elif interp_kind == "masked_gaussian":
                                    ...
                                    try:
                                        ...
                                        # sparse_map[mask_i] = 1
                                        from nvsmask3d.utils.utils import save_img
                                        # save_img(nvs_mask_img, f"tests/interp_object_{i}_camera_{interpolation_index}.png")
                                        # save_img(sparse_map, f"tests/interp_sparse_map_object_{i}_camera_{interpolation_index}.png")
                                        save_img(cropped_nvs_mask_image.permute(1,2,0), f"tests/interp_object_{i}_cropped_camera_{interpolation_index}.png")
                                        import pdb;pdb.set_trace()
                                    except Exception as e:
                                        print(f"Failed to save image {interpolation_index}: {e}")
                                        continue
        ...
        #################################################################################################GT################################################################################################
            ...
            if self.gt_camera_rgb or self.gt_camera_gaussian:
                ...
                for pose_index, index in zip(best_camera_indices, pose_sorted_index):
                    try:
                        if (index+1) > len(valid_u) or (index+1) > len(valid_v):
                            continue
            ...
            with torch.no_grad():
                ...
                if algorithm == -1:
                    ...
                    # features_num = mask_features.shape[0]
                ...
                if algorithm == 0:
                    # aggregate similarity scores 你目前是将批次中的相似度分数进行求和（sum），这可能会导致信息丢失，尤其是在增强视图之间存在较大差异的情况下。
                        ...
                        #     rgb_features, self.model.image_encoder.pos_embeds.T
                        # )
                        # rgb_features[: -(self.num_levels*self.top_k)]/=self.interpolate_n_rgb_camera
                        # scores = rgb_logits.sum(dim=0)
                    ...
                    if len(rgb_outputs) > 0:
                        ...
                        # scores = rgb_logits.sum(dim=0)
                        # features_num = rgb_features.shape[0]
                        # if features_num > self.top_k:                        
                        #     rgb_logits = torch.mm(
                        #         rgb_features, self.model.image_encoder.pos_embeds.T
                        #     )
                        #     if self.interpolate_n_camera > 0:
                        #         rgb_logits[: -self.top_k] /= (self.interpolate_n_camera)
                        #     scores = rgb_logits.sum(dim=0)
                        features_num = rgb_features.shape[0]
                        if features_num > self.top_k:
                            # 先做 mm，直接拿 rgb_features 和 pos_embeds 做相似度
                            # rgb_features: (num_features, D)
                            # pos_embeds: (num_pos_embeds, D) -> (D, num_pos_embeds)
                            rgb_logits = torch.mm(rgb_features, self.model.image_encoder.pos_embeds.T)  # (num_features, num_pos_embeds)

                            # 接下来你希望区分“主要视角 top_k 部分”和“interpolate 部分”：
                            # 先分离
                            if self.interpolate_n_camera > 0:
                                top_k_logits = rgb_logits[-self.top_k:]  # (top_k, num_pos_embeds)
                                interp_logits = rgb_logits[:-self.top_k]  # (features_num - top_k, num_pos_embeds)

                                # 先分别求和
                                f_sum = top_k_logits.sum(dim=0)  # (num_pos_embeds,)
                                if self.interpolate_n_camera > 0 and interp_logits.shape[0] > 0:
                                    alpha = 0.5  # 可调
                                    N = self.interpolate_n_camera
                                    interp_sum = interp_logits.sum(dim=0) * (alpha / N)
                                    f_sum = f_sum + interp_sum

                                # 归一化（如果需要）
                                #f_sum = f_sum / (f_sum.norm() + 1e-6)  # 防止除以零

                            # 最后在 f_sum 中取最大索引
                max_ind = torch.argmax(f_sum).item()
        ...
        for i in tqdm(range(cls_num), desc="Inferenceing objects", total=cls_num):
            ...
            with torch.no_grad():
                ...
                if algorithm == -1:
                    ...
                    if rgb_features.shape[0] > self.top_k:

                            rgb_logits[: -self.top_k] /= (rgb_features.shape[0] / (self.top_k))
                    scores = rgb_logits.sum(dim=0)
                ...
                if algorithm == 0:
                    ...
                    if len(rgb_outputs) > 0:
                        ...
                        if self.interpolate_n_camera > 0:
                            rgb_logits[: -self.top_k] /= (self.interpolate_n_camera*2)
                        scores = rgb_logits.sum(dim=0)
                if algorithm == 1:
                    scores = torch.mm(rgb_features.mean(dim=0).unsqueeze(0), model.image_encoder.pos_embeds.T)
