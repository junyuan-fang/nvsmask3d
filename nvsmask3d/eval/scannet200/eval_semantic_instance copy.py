...
# ---------- Label info ---------- #
DATASET_NAME = (
    "scannet"  # this is the default value, it is overridden if it is another dataset.
)
...
def evaluate_matches(matches):
    ...
    for di, (min_region_size, distance_thresh, distance_conf) in enumerate(
        ...
        for oi, overlap_th in enumerate(overlaps):
            ...
            try:
                WI[round(overlap_th, 2)] = FPo[round(overlap_th, 2)] / (
                    TPc[round(overlap_th, 2)] + FPc[round(overlap_th, 2)]
                )
            except:
                pass
    pcdc_scores = []
    ...
    for scene_name in gt_category_names.keys():
        pcdc_scores.append(
            np.asarray(
                [
                    len(matched_predictions_category_names[d_inf][oid][scene_name])
                    / len(gt_category_names[scene_name])
...
def compute_averages(aps, PRETRAINED_ON_SCANNET200=True):
    ...
    ):  # compute average scores for head, common, tail categories
            ...
            if not PRETRAINED_ON_SCANNET200:
                if label_name not in ["floor", "wall"]:
                    if label_name in BASE_CLASSES:
                        for ap_type in ["ap", "ap25%", "ap50%"]:
                            base_score[ap_type].append(
                                avg_dict["classes"][label_name][ap_type]
                            )
                    else:
                        for ap_type in ["ap", "ap25%", "ap50%"]:
                            novel_score[ap_type].append(
                                avg_dict["classes"][label_name][ap_type]
                            )
    if DATASET_NAME == "scannet200":
        ...
        if DATASET_NAME == "scannet200":
            if label_name in HEAD_CATS_SCANNET_200:
                for ar_type in ["ar", "ar25%", "ar50%"]:
                    head_scores[ar_type].append(
                        avg_dict["classes"][label_name][ar_type]
                    )
            elif label_name in COMMON_CATS_SCANNET_200:
                for ar_type in ["ar", "ar25%", "ar50%"]:
                    common_scores[ar_type].append(
                        avg_dict["classes"][label_name][ar_type]
                    )
            elif label_name in TAIL_CATS_SCANNET_200:
                for ar_type in ["ar", "ar25%", "ar50%"]:
                    tail_scores[ar_type].append(
                        avg_dict["classes"][label_name][ar_type]
                    )
            else:
                raise NotImplementedError(label_name)
        ...
        if DATASET_NAME == "scannet200":
            if label_name in HEAD_CATS_SCANNET_200:
                for rc_type in ["rc", "rc25%", "rc50%"]:
                    head_scores[rc_type].append(
                        avg_dict["classes"][label_name][rc_type]
                    )
            elif label_name in COMMON_CATS_SCANNET_200:
                for rc_type in ["rc", "rc25%", "rc50%"]:
                    common_scores[rc_type].append(
                        avg_dict["classes"][label_name][rc_type]
                    )
            elif label_name in TAIL_CATS_SCANNET_200:
                for rc_type in ["rc", "rc25%", "rc50%"]:
                    tail_scores[rc_type].append(
                        avg_dict["classes"][label_name][rc_type]
                    )
            else:
                raise NotImplementedError(label_name)
...
def print_results_ap_ar_rc_pcdc(
    avgs,
    ar_avgs,
    rc_avgs,
    pcdc_avgs,
    WI,
    A_OSE,
    print_mode={"ap_avgs": True, "ar_avgs": False, "rc_avgs": True, "pcdc_avgs": True},
):
    global DATASET_NAME
    sep = ""
    col1 = ":"
...
def evaluate_single_scene(preds: dict, gt_path: str, scene_name: str):
    ...
    if DATASET_NAME == "scannet200":
        CLASS_LABELS = (
            SCANNET200_CLASSES  # Assuming SCANNET200_CLASSES is defined elsewhere
        )
        VALID_CLASS_IDS = (
            SCANNET200_VALID_CLASS_IDS  # Assuming this is defined elsewhere
        )
    ...
    matches = {scene_name: {"gt": gt2pred, "pred": pred2gt}}
...
def main():
    ...
    print(
        pred_masks.shape, pred_scores.shape, pred_classes.shape
