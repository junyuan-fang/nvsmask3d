...
def get_instances_for_oracle(
    ...
    torch.save(
        np.asarray(new_masks).T,
        save_dir + scene_name + "_oracle_masks_" + dataset_name + ".pt",
    )
    torch.save(
        np.asarray(new_classes),
        save_dir + scene_name + "_oracle_classes_" + dataset_name + ".pt",
    )
    torch.save(
        np.ones(np.asarray(new_classes).shape),
...
def get_instances_for_oracle_replica(
    ids,
    class_ids,
    class_labels,
    id2label,
    ...
    print(save_dir + scene_name + "_oracle_masks_" + dataset_name + ".pt")
    torch.save(
        np.asarray(new_masks).T.astype(np.uint8),
        os.path.join(save_dir, scene_name + "_oracle_masks_" + dataset_name + ".pt"),
    )
    torch.save(
        np.asarray(new_classes),
        os.path.join(save_dir, scene_name + "_oracle_classes_" + dataset_name + ".pt"),
    )
