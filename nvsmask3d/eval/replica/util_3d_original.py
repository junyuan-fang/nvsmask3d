...
import nvsmask3d.eval.replica.util as util
...
def get_instances_for_oracle(ids, class_ids, class_labels, id2label, scene_name, save_dir='/media/ayca/Elements/ayca/OpenMask3D/oracle_scannet200_val/', dataset_name='scannet200'):
    import pdb

    instance_ids = np.unique(ids)

    assert (len(np.unique(ids))-1)>0, 'to-do: handle if len(np.unique(ids))-1)==0'
    num_instances = len(np.unique(ids))-1

    new_ids = ids.copy()
    new_pred_classes = np.zeros(num_instances).astype(int) #(33,)
    for new_id, old_id in enumerate(instance_ids):
        if new_id != 0:
            new_pred_classes[new_id-1] = Instance(ids, old_id).label_id
        new_ids[ids==old_id] = new_id
    
    #if inst.label_id in class_ids:
    new_pred_masks = np.zeros((new_ids.shape[0], num_instances))#(237360, 33)
    for idx in range(new_pred_masks.shape[1]):
        new_pred_masks[:, idx] = (new_ids==idx+1)

    new_masks = []
    new_classes = []
    for idx, lbl in enumerate(new_pred_classes):
        if lbl in class_ids:
            new_masks.append(new_pred_masks[:, idx])
            new_classes.append(lbl)

    import torch
    print(save_dir + scene_name + '_oracle_masks_'+dataset_name+'.pt')
    torch.save(np.asarray(new_masks).T, save_dir + scene_name + '_oracle_masks_'+dataset_name+'.pt')
    torch.save(np.asarray(new_classes), save_dir + scene_name + '_oracle_classes_'+dataset_name+'.pt')
    torch.save(np.ones(np.asarray(new_classes).shape), save_dir + scene_name + '_oracle_scores_'+dataset_name+'.pt')
...
def get_instances_for_oracle_replica(ids, class_ids, class_labels, id2label, scene_name, save_dir='/media/ayca/Elements/ayca/OpenMask3D/oracle_replica/', dataset_name='replica'):
    import pdb
    ...
    #pdb.set_trace()
    print(save_dir + scene_name + '_oracle_masks_'+dataset_name+'.pt')
    torch.save(np.asarray(new_masks).T.astype(np.uint8), os.path.join(save_dir, scene_name + '_oracle_masks_'+dataset_name+'.pt'))
    torch.save(np.asarray(new_classes), os.path.join(save_dir, scene_name + '_oracle_classes_'+dataset_name+'.pt'))
    #torch.save(np.ones(np.asarray(new_classes).shape), save_dir + scene_name + '_oracle_scores_'+dataset_name+'.pt')