...
# 场景数据路径
base_path = '/scratch/project_2003267/junyuan/data'
...
for scene_id in scene_ids:
    target_path = os.path.join(base_path, scene_id, 'dslr', 'render_depth')
    if not os.path.exists(target_path):
        missing_scenes.append(scene_id)

# 输出不存在的场景号
if missing_scenes:
    print("以下场景号的 'dslr/render_depth' 文件夹不存在:")
    print(missing_scenes)
else:
    print("所有场景号的 'dslr/render_depth' 文件夹都存在。")
